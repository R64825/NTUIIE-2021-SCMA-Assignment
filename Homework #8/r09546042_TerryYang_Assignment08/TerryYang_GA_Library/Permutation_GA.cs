...
namespace TerryYang_GA_Library
{
    ...
    public class Permutation_GA: Generic_GA_Solver<int>
    {
        ...
        {
            int mutation_Point = rnd.Next(0,number_Of_Genes);          

            // copy
            int[] remaining_Part = new int[number_Of_Genes - 1];
            int r_index = 0;
            for (int i = 0; i < number_Of_Genes ; i++)
            {
                if (mutation_Point != chromosomes[before_mutation][i])
                {
            ...
            // insert
            r_index = 0;
            int insertion_Point = rnd.Next(0, remaining_Part.Length);
            for (int i = 0; i < number_Of_Genes; i++)
            {
                ...
                {
                    chromosomes[after_mutation][i] = mutation_Point;
                }
            }
        ...
        private void Reciprocal_Exchange_Mutation(int before_mutation, int after_mutation, bool[] mutated_Flag)
        {
            ...
            // swap
            (chromosomes[after_mutation][p2], chromosomes[after_mutation][p1])
                = (chromosomes[before_mutation][p1], chromosomes[before_mutation][p2]);
